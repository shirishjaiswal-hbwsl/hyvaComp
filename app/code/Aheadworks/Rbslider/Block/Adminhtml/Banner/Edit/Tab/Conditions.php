...
class Conditions extends Generic implements TabInterface
{
    ...
    protected function getFormData()
    {
        if ($this->formData === null) {
            if (!empty($this->dataPersistor->get('aw_rbslider_banner'))) {
                $this->formData = $this->dataPersistor->get('aw_rbslider_banner');
            } elseif ($id = $this->getRequest()->getParam('id')) {
                $rule = $this->bannerRepository->get($id);
    ...
    private function addCategoryFieldset($form, $formData)
    {
        ...
        $categoryTreeBlock->setTemplate('Aheadworks_Rbslider::catalog/category/checkboxes/tree.phtml');
        ...
        if (isset($formData['category_ids'])) {
            $categoryTreeBlock->setCategoryIds(explode(',', (string)$formData['category_ids']));
        }
    ...
    <script type="text/javascript">
        ...
        Object.defineProperty(сategoryIds.updateElement, "value", {
            get: function() {
                return jQuery("#rule_category_ids").val();
            },
            set: function(v) {
